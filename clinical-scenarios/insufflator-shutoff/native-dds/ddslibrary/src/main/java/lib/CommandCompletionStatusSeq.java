...
/**
 * A sequence of CommandCompletionStatus instances.
 */
public final class CommandCompletionStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public CommandCompletionStatusSeq() {
        super(CommandCompletionStatus.class);
    }


    public CommandCompletionStatusSeq(int initialMaximum) {
        super(CommandCompletionStatus.class, initialMaximum);
    }


    public CommandCompletionStatusSeq(Collection elements) {
        super(CommandCompletionStatus.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, CommandCompletionStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' CommandCompletionStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CommandCompletionStatus.create());
